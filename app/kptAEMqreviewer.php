...
class kptAEMqreviewer extends Model
{
	/* Getting AEM data for KReviewer assigned list  */	
	public function aem_listing_assigned_qreviewer_results($user_id){				
  ...
  /* AEM xml files request */ 
	...
	/* AEM request getting for Source text */
	public function getting_aem_request_sourcetext_translation_select($aem_reference_code){		
		$queryResult = DB::select('call xp_loc_aem_request_sourcetext_translation_select(?)',array($aem_reference_code));
		$result = collect($queryResult);		
		return $result;	
	}
	/* AEM request getting for Source text */
	...
	public function aem_request_update_kpt_reviewer_text_row_by_row($arr_import_translation){		
		...
		$queryResult = DB::select('call xp_loc_aem_request_update_qatar_reviewer_text_row_by_row(?,?,?,?,?,?,?)',array($arr_import_translation[0],$arr_import_translation[1],$arr_import_translation[2],$arr_import_translation[3],$arr_import_translation[4],$arr_import_translation[5],$arr_import_translation[6]));
		$result = collect($queryResult);	
		return $result;		
	}
