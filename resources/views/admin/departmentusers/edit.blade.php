...
<div class="card">
    ...
    <div class="card-body">
        ...
        <form action="{{ route("admin.departmentusers.update", [$DepartmentUsers->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                <label for="name" class="required">{{ trans('cruds.user.fields.name') }}</label>
                <input type="text" id="name" name="name" class="form-control" value="{{ old('name', isset($DepartmentUsers) ? $DepartmentUsers->name : '') }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                ...
                <input type="email" id="email" name="email" class="form-control" value="{{ old('email', isset($DepartmentUsers) ? $DepartmentUsers->email : '') }}" required>
                @if($errors->has('email'))
                    <em class="invalid-feedback">
                        {{ $errors->first('email') }}
                    </em>
			...
			<!-- Department block -->
			<div class="form-group {{ $errors->has('roles') ? 'has-error' : '' }}">
                <label for="roles" class="required">Sub Department</label>
                 <select name="department_id" id="department_list" class="form-control select2"  required>
