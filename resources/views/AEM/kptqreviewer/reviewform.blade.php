...
<div class="card">
	...
	<form id="frm_segment" action="{{ route("admin.kptaemqreviewerrequests.aem_request_qatar_proofreading_submit") }}" method="POST" enctype="multipart/form-data">
	 @csrf
    ...
    <div class="card-body">
        <div class="table-responsive">					
           <?php						
			...
			$arr_aem_translated_strings = $kptaemqreviewer->getting_translated_string_aem_request($aem_reference_code,$aem_object_id,$aem_source_language,$aem_target_language);
		...
		<?php
			...
			if(!empty($arr_aem_translated_strings)){
					...
					foreach($arr_aem_translated_strings as $t_rows){					
						 ...
						 $reviewer_comments  =  $t_rows->qatar_reviewer_comments;
						 ...
						 $translation_success="";
						 $translaton_failure="";
						if($qatar_reviewer_status ==1)
							$translation_success ="checked";
						elseif($qatar_reviewer_status == 0)
   ...
   <?php		
		$arr_review_status = array("Assigned Qatar Review","Qatar Review Failed");
